 ...
 *   o importado como módulo desde server.js
 */
...
async function scrapeBumeran(browser, keyword, location = 'argentina') {
  const results = [];
  ...
  try {
    ...
    stmts.logScrape.run({ source: 'bumeran', keyword, found: 0, new_jobs: 0, error: err.message });
  } finally {
    await page.close();
  }
...
async function scrapeIndeed(browser, keyword, location = 'Argentina') {
  const results = [];
  const page = await browser.newPage();
  ...
  try {
    ...
    stmts.logScrape.run({ source: 'indeed', keyword, found: 0, new_jobs: 0, error: err.message });
  } finally {
    await page.close();
  }
...
async function scrapeLinkedIn(browser, keyword, location = 'Argentina') {
  const results = [];
  const page = await browser.newPage();
  ...
  try {
    ...
    stmts.logScrape.run({ source: 'linkedin', keyword, found: 0, new_jobs: 0, error: err.message });
  } finally {
    await page.close();
  }
...
async function scrapeComputrabajo(browser, keyword, location = '') {
  const results = [];
  const page = await browser.newPage();
  ...
  try {
    ...
    stmts.logScrape.run({ source: 'computrabajo', keyword, found: 0, new_jobs: 0, error: err.message });
  } finally {
    await page.close();
  }
...
function saveJobs(jobs) {
  ...
  const insertMany = db.transaction((list) => {
    for (const job of list) {
      const tags = JSON.stringify(inferTags(job.title, job.description || ''));
...
async function runScrape(options = {}) {
  ...
  const keywords = options.keywords
    || JSON.parse(profile?.keywords || '["frontend developer"]');
  ...
  for (const keyword of keywords) {
    for (const source of sources) {
      ...
      try {
        const jobs = await scraperMap[source](context, keyword, location);
        allJobs.push(...jobs);
  ...
  const newCount = saveJobs(allJobs);
...
if (require.main === module) {
  const keyword = process.argv[2] || 'frontend developer';
  const location = process.argv[3] || process.env.SEARCH_COUNTRY || 'Argentina';
  runScrape({ keywords: [keyword], location }).catch(console.error);
}
